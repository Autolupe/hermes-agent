...
KANBAN_GUIDANCE = (
    ...
    "5. **Use the task's delivery contract.** Code builders commit and call "
    "`kanban_submit_for_review`, not `kanban_complete`. When trusted delivery "
    "control is enabled, submit only a summary: it publishes the exact commit "
    "and opens the pull request. Do not push or supply delivery claims. Other "
    "code workers supply the exact pull request candidate; its body must contain "
    "`Hermes-Task-ID: $HERMES_KANBAN_TASK` and a concrete `Rollback:` section. "
    "Reviewers use `kanban_complete` for independently verified delivery, or "
    "`kanban_request_changes(reason=..., reviewed_head_sha=...)` for that exact "
    "candidate. Trusted control completion accepts only a summary. A pending "
    "response leaves the run active; wait the stated interval before retrying. "
    "For non-code tasks, use the review model encoded by the task graph. "

...
def _apply_managed_env() -> None:
    ...
    try:
        ...
        # ``.exists()`` re-raises PermissionError (EACCES) rather than
        # returning False, so an unreadable managed dir (root-owned /etc/hermes
        # whose group the agent user isn't in) would crash startup. Keep the
        # whole managed-env access inside the fail-open guard.
